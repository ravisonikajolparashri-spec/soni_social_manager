          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="px-4 py-3.5">
                      <span className="font-mono text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md">#{u.id}</span>
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5">
                      {u.is_admin && (
                        <span className="bg-violet-100 text-violet-700 border border-violet-200 px-2.5 py-1 rounded-full text-[11px] font-semibold">
                          Admin
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex gap-1.5">
                        <button
                          ...
                          className="text-xs bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-lg hover:bg-emerald-200 font-medium transition-colors"
                        >
                          Add Funds
                        </button>
                        {!u.is_admin && (
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} className="text-center py-12 text-slate-400 text-sm">No users found</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <form onSubmit={handleAddFunds} className="space-y-4">
              ...
              <div className="flex gap-3">
                <button type="submit" className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl text-sm font-semibold transition-colors">
                  Add Funds
                </button>
                <button type="button" onClick={() => setFundModal(null)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 py-2.5 rounded-xl text-sm font-medium transition-colors">
                  Cancel
                </button>
              </div>
            </form>
